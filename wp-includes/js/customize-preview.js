    ...
    */
    ...
    initialize: function ( params, options ) {
        ...
        if (api.settings.channel ) {
            ...
            preview.body.on(
                'click.preview', 'a', function ( event ) {
                    preview.handleLinkClick(event);
                } 
            );
            preview.body.on(
                'submit.preview', 'form', function ( event ) {
                    preview.handleFormSubmit(event);
                } 
            );

            preview.window.on(
                'scroll.preview', debounce(
                    function () {
    ...
    api.addLinkPreviewing = function addLinkPreviewing()
    {
        ...
        if ('undefined' !== typeof MutationObserver ) {
            ...
            $(document.documentElement).on(
                'click focus mouseover', linkSelectors, function () {
                    api.prepareLinkPreview(this);
                } 
            );
        }
    ...
    api.isLinkPreviewable = function isLinkPreviewable( element, options )
    {
        ...
        matchesAllowedUrl = ! _.isUndefined(
            _.find(
                api.settings.url.allowed, function ( allowedUrl ) {
    ...
    api.prepareFormPreview = function prepareFormPreview( form )
    {
        ...
        _.each(
            stateParams, function ( value, name ) {
    ...
    api.keepAliveCurrentUrl = ( function () {
        ...
        return function keepAliveCurrentUrl()
        {
            ...
            if (null === previousQueryParams ) {
                ...
                _.each(
                    stateQueryParams, function ( name ) {
    ...
    api.settingPreviewHandlers = {
        ...
        background: function () {
            ...
            _.each(
                ['color', 'image', 'preset', 'position_x', 'position_y', 'size', 'repeat', 'attachment'], function ( prop ) {
    ...
    $(
        function () {
            ...
            $.each(
                api.settings._dirty, function ( i, id ) {
                    var setting = api(id);
                    if (setting ) {
                        setting._dirty = true;
                    }
                ...
                'sync', function ( events ) {
                    ...
                    */
                    if (events.settings && events['settings-modified-while-loading'] ) {
                        _.each(
                            _.keys(events.settings), function ( syncedSettingId ) {
            ...
            */
            api.preview.bind(
                'changeset-saved', function ( data ) {
                    _.each(
                        data.saved_changeset_values, function ( value, settingId ) {
            ...
            */
            ...
            api.preview.bind(
                'loading-initiated', function () {
                    $('body').addClass('wp-customizer-unloading');
                }
            );
            api.preview.bind(
                'loading-failed', function () {
            ...
            api(
                'custom_css[' + api.settings.theme.stylesheet + ']', function ( setting ) {
