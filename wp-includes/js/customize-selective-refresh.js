...
wp.customize.selectiveRefresh = ( function ( $, api ) {
    ...
    ready: function () {
        ...
        $(document).on(
        'click', partial.params.selector, function ( e ) {
            if (! e.shiftKey ) {
                return;
            }
            e.preventDefault();
        ...
        self._debouncedTimeoutId = setTimeout(
            function () {
                ...
                */
                ...
                _.each(
                    self._pendingPartialRequests, function ( pending, partialId ) {
                        ...
                        if (! self.partial.has(partialId) ) {
                            ...
                            * the contained sidebar for rendering widgets or what the nav menu args are for a menu.
                            ...
                            partialPlacementContexts[ partialId ] = _.map(
                                partialsPlacements[ partialId ], function ( placement ) {
                ...
                request = self._currentRequest = wp.ajax.send(
                    null, {
                        data: data,
                        ...
                        _.each(
                            self._pendingPartialRequests, function ( pending, partialId ) {
                                ...
                                if (! _.isArray(data.contents[ partialId ]) ) {
                                    ...
                                    placementsContents = _.map(
                                        data.contents[ partialId ], function ( content, i ) {
    ...
    api.bind(
        'preview-ready', function () {
            ...
            _.each(
                self.data.partials, function ( data, id ) {
             ...
             */
            api.selectiveRefresh.bind(
                'render-partials-response', function handleSettingValiditiesResponse( data )
                {
            ...
            api.preview.bind(
                'edit-shortcut-visibility', function ( visibility ) {
                    api.selectiveRefresh.editShortcutVisibility.set(visibility);
                } 
            );
            api.selectiveRefresh.editShortcutVisibility.bind(
                function ( visibility ) {
