...
function is_blog_installed()
{
    ...
    */
    ...
    foreach ( $wp_tables as $table ) {
        ...
        if (( ! $described_table && empty($wpdb->last_error) ) 
...
function dead_db()
{
    ...
    if (file_exists(WP_CONTENT_DIR . '/db-error.php') ) {
        include_once WP_CONTENT_DIR . '/db-error.php';
        die();
    }
 ...
 *                               to anonymize it are not present. Default false, return `::` (unspecified
 *                               address).
 ...
 * @param  int          $max_execution_time Maximum time to run before giving up. In seconds.
 *                                          The timeout is global and is measured from the
 *                                          moment WordPress started to load.
 * @return int|false|null Size in bytes if a valid directory. False if not. Null if timeout.
 */
