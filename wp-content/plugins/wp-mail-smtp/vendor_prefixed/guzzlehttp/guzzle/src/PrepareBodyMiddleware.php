...
class PrepareBodyMiddleware
{
    ...
    public function __invoke(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options)
    {
        ...
        if (!$request->hasHeader('Content-Type')) {
            if ($uri = $request->getBody()->getMetadata('uri')) {
                if ($type = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\mimetype_from_filename($uri)) {
     ...
     * @return void
     */
    private function addExpectHeader(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options, array &$modify)
    {
