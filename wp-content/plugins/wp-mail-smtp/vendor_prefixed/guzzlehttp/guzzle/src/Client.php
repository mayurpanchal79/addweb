...
class Client implements \WPMailSMTP\Vendor\GuzzleHttp\ClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\uri_for($config['base_uri']);
        }
     ...
     * @return Promise\PromiseInterface
     */
    public function sendAsync(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
     ...
     */
    public function send(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
    ...
    private function buildUri($uri, array $config)
    {
        ...
        $uri = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\uri_for($uri === null ? '' : $uri);
        if (isset($config['base_uri'])) {
            $uri = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\UriResolver::resolve(\WPMailSMTP\Vendor\GuzzleHttp\Psr7\uri_for($config['base_uri']), $uri);
        }
    ...
    {
        $defaults = ['allow_redirects' => \WPMailSMTP\Vendor\GuzzleHttp\RedirectMiddleware::$defaultSettings, 'http_errors' => \true, 'decode_content' => \true, 'verify' => \true, 'cookies' => \false, 'idn_conversion' => \true];
     ...
     * @return Promise\PromiseInterface
     */
    private function transfer(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options)
    {
        // save_to -> sink
...
*/
     ...
     */
    private function applyOptions(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array &$options)
    {
        $modify = ['set_headers' => []];
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (isset($options['json'])) {
            $options['body'] = \WPMailSMTP\Vendor\GuzzleHttp\json_encode($options['json']);
            unset($options['json']);
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\_caseless_remove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== \true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\_caseless_remove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\stream_for($options['body']);
            unset($options['body']);
        }
        ...
        $request = \WPMailSMTP\Vendor\GuzzleHttp\Psr7\modify_request($request, $modify);
