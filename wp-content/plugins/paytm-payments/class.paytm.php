...
class WC_paytm extends WC_Payment_Gateway {
	...
	public function generate_paytm_form($order_id){
		...
		foreach($woocommerce->cart->cart_contents as $key=>$ProductArr)
		{
			$tablename = $wpdb->prefix.'wc_order_detail';
			$wpdb->insert( $tablename, array(
				'order_id' => $order_id, 
				'product_id' => $woocommerce->cart->cart_contents[$key]['product_id'], 
				'FullName' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['FullName'], 
				'Message' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Message'],
				'Addressline1' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Addressline1'], 
				'Addressline2' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Addressline2'],
				'City' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['City'],
				'Pincode' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Pincode'], 
				'MobileNumber' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['MobileNumber'], 
				'Email' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Email'],
				'Delivery' => $woocommerce->cart->cart_contents[$key]['extra_product_data']['Delivery']
				)
			);
		}
