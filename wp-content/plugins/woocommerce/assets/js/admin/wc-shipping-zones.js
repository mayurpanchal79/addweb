            ...
            ShippingZone       = Backbone.Model.extend(
                {
                    ...
                    save: function () {
                        if (_.size(this.changes) ) {
                            $.post(
                                ajaxurl + ( ajaxurl.indexOf('?') > 0 ? '&' : '?' ) + 'action=woocommerce_shipping_zones_save_changes', {
            ...
            ShippingZoneView = Backbone.View.extend(
                {
                    ...
                    renderShippingMethods: function ( zone_id, shipping_methods ) {
                        ...
                        if (_.size(shipping_methods) ) {
                            shipping_methods = _.sortBy(
                                shipping_methods, function ( method ) {
                                    return parseInt(method.method_order, 10);
                                } 
                            );

                                _.each(
                                    shipping_methods, function ( shipping_method ) {
