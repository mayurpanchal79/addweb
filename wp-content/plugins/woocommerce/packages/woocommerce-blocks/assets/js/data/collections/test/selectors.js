...
describe(
    'getCollection', () => {
        it(
        'returns empty array when namespace does not exist in state', () => {
                expect(getCollection(state, 'invalid', 'products')).toEqual([]);
        } 
    );
    it(
        'returns empty array when resourceName does not exist in state', () => {
        expect(getCollection(state, 'wc/blocks', 'invalid')).toEqual([]);
        } 
    );
    it(
        'returns empty array when query does not exist in state', () => {
        expect(getCollection(state, 'wc/blocks', 'products')).toEqual([]);
        } 
    );
    it(
        'returns empty array when ids do not exist in state', () => {
        expect(
                getCollection(
                state,
                'wc/blocks',
    ...
    describe(
        'returns expected values for items existing in state', () => {
        test.each`
...
describe(
    ...
    it(
        'returns null when there are no headers for the given arguments', () => {
    ...
    it(
        'returns expected header when it exists', () => {
        expect(
                getCollectionHeader(
                state, 'total', 'wc/blocks', 'products', {
                        someQuery: 2,
                    } 
            )
            ).toBe(22);
